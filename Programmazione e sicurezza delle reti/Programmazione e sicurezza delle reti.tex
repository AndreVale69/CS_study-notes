...
CMD ["java", "-jar", "myapp.jar"]\end{lstlisting}
	...
	\subsubsection{Struttura del topic}
	
	I \textcolor{Red3}{\textbf{\emph{topic}}} (argomenti) servono ad \textbf{etichettare i dati per indicarne la tipologia}. Solitamente sono stringhe di testo in cui ci sono spazi, anche se è preferibile utilizzare trattini \dquotes{-} o \dquotes{\_}.\newline
	
	\noindent
	Inoltre, è possibile creare \textbf{gerarchie di argomenti} utilizzando lo \emph{slash} \dquotes{/}. Come per \textcolor{Green4}{\textbf{esempio}}:
	\begin{itemize}
		\item \textsf{casa/sensori/temperatura/cucina}
		
		\item \textsf{casa/sensori/temperatura/soggiorno}
	\end{itemize}
	Qualsiasi \emph{topic} compreso tra \dquotes{/} si chiama \textbf{livello}.\newline
	
	\noindent
	I \textbf{subscriber} possono utilizzare i caratteri \dquotes{\#} e \dquotes{+} come sostituti di parti di \emph{topic} (wildcards, si veda il prossimo paragrafo).
	
	\longline
	
	\subsubsection{Approfondimento Wildcards}
	
	Gli \textcolor{Red3}{\textbf{wildcards}} sono simboli che vengono utilizzati dai \emph{subscribers} al posto dei topic per avere risultati più completi:
	\begin{itemize}
		\item Il \textbf{carattere} \dquotes{\#} viene utilizzato per \textbf{sottoscriversi a tutti i topic} che si ottengono sostituendo \# con una sequenza di \underline{\textbf{livelli di suffisso reali}}.
		
		Ad \textcolor{Green4}{\textbf{esempio}}, invece di sottoscriversi ai topic:
		\begin{itemize}
			\item \textsf{level1/level2/level3}
			\item \textsf{level1/level2/level3bis}
		\end{itemize}
		Basta sottoscriversi a:
		\begin{itemize}
			\item \textsf{level1/level2/\#}
		\end{itemize}
		
		\item Il \textbf{carattere} \dquotes{+} viene utilizzato per \textbf{sottoscriversi a tutti i topic} che si ottengono sostituendo + con una sequenza di \underline{\textbf{livelli di prefisso reali}}.
		...
		Basta sottoscriversi a:
		\begin{itemize}
			\item \textsf{+/level2/level3}
		\end{itemize}
	\end{itemize}
	
	\longline
	
	\subsubsection{Approfondimento Broker}\label{par: Approfondimento Broker}
	
	Il \textcolor{Red3}{\textbf{\emph{broker}}} è l'\textbf{intermediario} nel modello Pub/Sub ed è l'\underline{unico} che \textbf{implementa connessioni TCP lato server} (porta 1883: trasmissioni in chiaro; porta 8883: trasmissioni TCP con sicurezza TLS/SSL). Inoltre, il \textbf{broker} può trovarsi su \textbf{hardware} e \textbf{sistemi operativi diversi} da quelli dei pub/sub. Anche il \textbf{linguaggio di programmazione} utilizzato può essere \textbf{differente}. Questo perché il protocollo \textbf{MQTT} che \textbf{consente di coordinare tutto}.\newpage
	
	\subsection[\textcolor{Red3}{\textbf{Esercizio}}]{Esercizi MQTT}
	
	\subsubsection{Guida all'installazione di Mosquitto su Linux}
...
sudo apt install mosquitto\end{lstlisting}
	...
	\begin{lstlisting}
sudo systemctl start mosquitto\end{lstlisting}
...
sudo systemctl status mosquitto\end{lstlisting}
	...
	\begin{lstlisting}
sudo systemctl stop mosquitto\end{lstlisting}
...
sudo apt install mosquitto-clients\end{lstlisting}
	...
	\begin{table}[!htp]
		...
		\begin{tabular}{@{} l l @{}}
			...
			Descrizione & Comando \\
			\midrule
			Indirizzo IP broker & \textsf{-h} oppure \textsf{-}\textsf{-host} \\
			Topic messaggio 	& \textsf{-t} oppure \textsf{-}\textsf{-topic} \\
			Contenuto messaggio & \textsf{-m} oppure \textsf{-}\textsf{-message} \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\subsubsection{Domande sull'esempio Pub/Sub}
	
	Dato l'esempio:
	\begin{itemize}
		\item \textsf{mosquitto\_sub -t "temperatura"}
		\item \textsf{mosquitto\_pub -m "22" -t "temperatura"}
	\end{itemize}
	Si risponda alle seguenti domande.\newline
	
	\noindent
	\textcolor{Green4}{\textbf{\emph{Partendo dall'esercizio mostrato nelle istruzioni, cosa succede se pubblico una temperatura prima di aver lanciato il subscriber?}}}\newline
	...
	\noindent
	\textcolor{Green4}{\textbf{\emph{Partendo dall'esercizio mostrato nelle istruzioni, creare un'applicazione pub/sub con 2 sensori di temperatura relativi a 2 stanze diverse. Quante finestre di terminale devo aprire?}}}\newline
	...
	\begin{itemize}
		\item Subscribers:
		\begin{enumerate}
			\item Terminale \dquotes{archivio} della prima stanza:
			\begin{lstlisting}
mosquitto_sub -t "temperatura/stanza1"\end{lstlisting}
			
			\item Terminale \dquotes{archivio} della seconda stanza
			\begin{lstlisting}
mosquitto_sub -t "temperatura/stanza2"\end{lstlisting}
		\end{enumerate}
		
		\item Publisher:
		\begin{enumerate}
			\item Terminale \dquotes{sensore di temperatura} delle due stanze. Quindi verrà eseguita una pubblicazione prima di una stanza:
			\begin{lstlisting}
mosquitto_pub -m "22" -t "temperatura/stanza1"\end{lstlisting}
			E poi dell'altra:
			\begin{lstlisting}
mosquitto_pub -m "22" -t "temperatura/stanza2"\end{lstlisting}
		\end{enumerate}
	\end{itemize}
...
mosquitto_sub -t "temperatura/stanza2"\end{lstlisting}
		...
		\item Publishers:
		\begin{enumerate}
			\item Terminale \dquotes{sensore di temperatura} della prima stanza. Quindi verrà eseguita una pubblicazione della stanza numero 1:
			\begin{lstlisting}
mosquitto_pub -m "22" -t "temperatura/stanza1"\end{lstlisting}

			\item Terminale \dquotes{sensore di temperatura} della seconda stanza. Quindi verrà eseguita una pubblicazione della stanza numero 2:
			\begin{lstlisting}
mosquitto_pub -m "22" -t "temperatura/stanza2"\end{lstlisting}	
		\end{enumerate}
	\end{itemize}\newpage
	...
	\noindent
	\textcolor{Green4}{\textbf{\emph{Partendo dall'esercizio precedente, come fare per avere un unico subscriber per entrambe le temperature? Come si fa a distinguere da quale stanza proviene la temperatura?}}}\newline
	...
	\begin{itemize}
		\item Subscriber:
		\begin{enumerate}
			\item Terminale \dquotes{archivio unico} di entrambe le stanze:
			\begin{lstlisting}
mosquitto_sub -t "temperatura/#"\end{lstlisting}
		\end{enumerate}
		...
		\begin{enumerate}
			\item Terminale \dquotes{sensore di temperatura} della prima stanza. Quindi verrà eseguita una pubblicazione della stanza numero 1:
			\begin{lstlisting}
mosquitto_pub -m "22-stanza1" -t "temperatura/stanza1"\end{lstlisting}
			
			\item Terminale \dquotes{sensore di temperatura} della seconda stanza. Quindi verrà eseguita una pubblicazione della stanza numero 2:
			\begin{lstlisting}
mosquitto_pub -m "22-stanza2" -t "temperatura/stanza2"\end{lstlisting}	
		\end{enumerate}
	\end{itemize}
	...
	\noindent
	\textcolor{Green4}{\textbf{\emph{Usare Wireshark per capire quale protocollo viene utilizzato da tutti gli attori a livello 3 e 4. Quali sono le porte di livello trasporto coinvolte?}}}\newline
	
	\noindent
	Come protocollo viene utilizzato MQTT:
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/pub-sub/esercizi-1}
	\end{figure}
	...
	\begin{itemize}
		\item Il subscriber utilizza la porta 38120;
		
		\item Il publisher della stanza 1 utilizza la porta 54474;
		
		\item Il publisher della stanza 2 utilizza la porta 60688.
	\end{itemize}
	La porta utilizzata dal broker, visibile poiché il pacchetto 16, 30 e 48 comunicano con lui e viceversa, è la numero 1883.\newpage
	
	\noindent
	\textcolor{Green4}{\textbf{\emph{Prova a pubblicare un valore di umidità relativa (topic \dquotes{UR}); il subscriber interessato alla temperatura lo riceve? Come si fa a creare un subscriber interessato all'umidità? Costruire un'applicazione pub/sub con 4 finestre per produrre e visualizzare sia valori di temperatura sia valori di umidità.}}}\newline
	...
	\begin{lstlisting}
mosquitto_sub -t "UR"\end{lstlisting}
	Per costruire un'applicazione pub/sub con 4 finestre:
	\begin{itemize}
		\item Publishers:
		\begin{enumerate}
			\item Terminale \dquotes{sensore di temperatura}. Quindi verrà eseguita una pubblicazione del tipo:
			\begin{lstlisting}
mosquitto_pub -m "22" -t "temperatura"\end{lstlisting}
			
			\item Terminale \dquotes{sensore di umidità}. Quindi verrà eseguita una pubblicazione del tipo:
			\begin{lstlisting}
mosquitto_pub -m "5" -t "UR"\end{lstlisting}
		\end{enumerate}
		
		\item Subscribers:
		\begin{enumerate}
			\item Terminale \dquotes{archivio temperature}:
			\begin{lstlisting}
mosquitto_sub -t "temperatura"\end{lstlisting}
				
			\item Terminale \dquotes{archivio umidità}:
			\begin{lstlisting}
mosquitto_sub -t "UR"\end{lstlisting}
		\end{enumerate}
	\end{itemize}\newpage
	...
	\noindent
	\textcolor{Green4}{\textbf{\emph{Si vuole costruire con MQTT un servizio di messaggistica universitaria:}}}
	\begin{itemize}
		\item \textcolor{Green4}{\textbf{\emph{Il rettore può leggere tutti i messaggi}}}
		
		\item \textcolor{Green4}{\textbf{\emph{La segreteria può leggere i messaggi dai docenti e dagli studenti}}}
		
		\item \textcolor{Green4}{\textbf{\emph{I docenti possono leggere i messaggi dai docenti e dagli studenti}}}
		
		\item \textcolor{Green4}{\textbf{\emph{Gli studenti possono leggere solo i messaggi degli altri studenti}}}
	\end{itemize}
	...
	\begin{itemize}
		\item Livello 1:
		\begin{itemize}
			\item \textsf{universita}
		\end{itemize}
		
		\item Livello 2:
		\begin{itemize}
			\item \textsf{universita/personale}
			\item \textsf{universita/direzione}
		\end{itemize}
		
		\item Livello 3:
		\begin{itemize}
			\item \textsf{universita/personale/docenti}
			\item \textsf{universita/personale/studenti}
		\end{itemize}
	\end{itemize}
	Adesso si impostano i 4 subscribers in 4 terminali diversi:
	\begin{itemize}
		\item Il rettore ha i permessi di leggere qualsiasi messaggio dell'università, quindi deve avere accesso a qualsiasi topic:
		\begin{lstlisting}
mosquitto_sub -t "universita/#"\end{lstlisting}
		
		\item La segreteria deve avere la possibilità di leggere solo i messaggi provenienti dai docenti e dagli studenti:
		\begin{lstlisting}
mosquitto_sub -t "universita/personale/#"\end{lstlisting}
		
		\item I docenti hanno gli stessi permessi della segreteria e quindi il comando sarà:
		\begin{lstlisting}
mosquitto_sub -t "universita/personale/#"\end{lstlisting}
		
		\item Gli studenti hanno i permessi più restrittivi di tutti, ovvero possono solo leggere i messaggi provenienti da altri studenti:
		\begin{lstlisting}
mosquitto_sub -t "universita/personale/studenti"\end{lstlisting}
	\end{itemize}\newpage
	
	\subsubsection[\textcolor{Red3}{\textbf{Possibile domanda da esame}}]{Possibile domanda da esame}
	
	Durante le esercitazioni fatte in aula, il professore si è raccomandato più e più volte di disattivare Mosquitto una volta terminati gli esercizi. Addirittura è stato consigliato di disinstallarlo se non utilizzato per molto tempo. La domanda, interessante e papabile per l'esame, è la seguente: perché Mosquitto deve essere disattivato per motivi di sicurezza?\newline
	
	\noindent
	Mosquitto è un message broker open source che implementa il protocollo MQTT. Tale servizio, una volta installato, viene eseguito ogniqualvolta il sistema si avvia.
	
	Quando Mosquitto viene avviato, in automatico all'avvio o manualmente da linea di comando, sulla macchina viene aperta una porta (1883 oppure 8883, come descritto nel paragrafo a pagina~\pageref{par: Approfondimento Broker}). Dunque, il calcolatore diventa un vero e proprio server e infatti esso è un vero e proprio \emph{broker} che smista i messaggi nei vari topic.\newline
	
	\noindent
	\textbf{La disattivazione comporta la chiusura della porta e si evita che qualche attaccante possa sfruttare questa falla per entrare nel sistema o diffondere un virus.}\newpage
	...
	\subsection[\textcolor{Red3}{\textbf{Domande da esame}}]{Domande da esame}
	
	Possibili domande da esame:
	\begin{enumerate}
		\item \textcolor{Green4}{\textbf{\emph{Perché è necessario certificare un sito web? Perché nel nome del certificato c'è il nome del sito?}}}
		
		\textbf{Il certificato è essenziale poiché certifica il dominio di un determinato sito web} e \textbf{inoltre fornisce una chiave pubblica utilizzata da chi visita il sito, per inviare un messaggio cifrato}. Così facendo, tutti i malintenzionati potranno creare qualsiasi sito di phishing, ma non avranno mai la certificazione ufficiale rilasciata dall'ente certificato.
		
		È possibile che un attaccante esegua:
		\begin{itemize}
			\item Un attacco ad un server DNS;
			
			\item Crei un server DNS fasullo.
		\end{itemize}
		In entrambi i casi, l'utente potrebbe cadere in trappola. Infatti, nel momento in cui vada a richiedere un sito al server DNS, credendo che sia quello originale, otterrebbe un indirizzo IP di un sito web falso. Nonostante il \emph{fake website} possa avere una certificazione, quest'ultima potrebbe \underline{non} essere rilasciata da un ente certificato e avere un nome falso.
		
		\item \textcolor{Green4}{\textbf{\emph{È strettamente obbligatorio un certificato lato client?}}}
		
		Se la connessione è HTTPS, la connessione è cifrata con una chiave simmetrica. Quindi il client non necessita di un certificato, ma può banalmente utilizzare username e password.
		
		Nel caso in cui un attaccante si metta ad ascoltare la conversazione (Man-in-the-middle), non potrebbe avere speranze di leggere in chiaro la conversazione poiché sarebbe criptata. Cosa invece che non succede nel caso di HTTP.
	\end{enumerate}\newpage
...
npm run start\end{lstlisting}
	...
	L'applicazione WePlant utilizza una configurazione \textcolor{Red3}{\textbf{Design Responsive}}.\newline
	
	\noindent
	Con Design Responsive si intende una configurazione in cui il \textbf{server invia sempre lo stesso codice HTML a tutti i dispositivi e il codice CSS viene utilizzato per modificare il \emph{rendering} della pagina} sul dispositivo. I \textcolor{Green4}{\textbf{vantaggi}}:
	\begin{itemize}
		\item \textbf{Creazione di siti/applicazioni web che si adattano al dispositivo su cui vengono visualizzate} (PC, tablet, smartphone) in base alla dimensione dello schermo.
		
		\item \textbf{Ottimizzazione dei motori di ricerca}, ovvero dell'indicizzazione delle pagine (SEO, Search Engine Optimization). Le pagine \emph{responsive} ottengono un punteggio più alte e vengono visualizzate come primi risultati.
		
		\item \textbf{Tempo di progettazione inferiore} per la manutenzione di più pagine per gli stessi contenuti.
		
		\item \textbf{Nessun reindirizzamento per offrire agli utenti una visualizzazione ottimizzata} in base al dispositivo in uso.
	\end{itemize}
	Per \textbf{segnalare al browser che la pagina è Design Responsive}, viene utilizzato un meta tag nell'intestazione chiamato \textbf{meta tag viewport}. Nel caso in cui mancasse, i browser per dispositivi mobili applicano l'impostazione predefinita, ovvero eseguono il rendering della pagina alla larghezza utilizzata per gli schermi desktop.
\end{document}