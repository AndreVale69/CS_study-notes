...
drop table Dipartimento cascade\end{lstlisting}
	...
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/schema-concettuale.png}
	\end{figure}
...
SELECT id, nomeins FROM Insegn WHERE nomeins='Algoritmi'\end{lstlisting}
	...
	\lstinputlisting[language=SQL]{code/index.sql}
	\begin{itemize}
	...
	\lstinputlisting[language=SQL]{code/index-ex1.sql}
	...
	\lstinputlisting[language=SQL]{code/index-ex2.sql}
	...
	\lstinputlisting[language=SQL]{code/index-ex3.sql}\:\newline
	...
	\lstinputlisting[language=SQL]{code/index-ex4.sql}
	...
	\lstinputlisting[language=SQL]{code/index-ex5.sql}
	...
	\lstinputlisting[language=SQL]{code/index-ex6.sql}
	...
	\lstinputlisting[language=SQL]{code/index-ex7.sql}\newpage
...
EXPLAIN SELECT * FROM Insegn\end{lstlisting}
	...
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-1.pdf}
	\end{figure}
	\begin{itemize}
...
EXPLAIN SELECT * FROM Insegn WHERE id < 1000\end{lstlisting}
	...
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-2.pdf}
	\end{figure}
	\begin{itemize}
	...
	\noindent
	Un altro \textcolor{Green4}{\textbf{esempio}} di \textsf{explain} con \textbf{due nodi}:
	\begin{lstlisting}[language=SQL]
DROP INDEX ins_nomeins; -- c'e' solo indice su id
EXPLAIN SELECT * FROM Insegn WHERE id < 1000 AND nomeins LIKE 'Al%'\end{lstlisting}
	\emph{\underline{Output:}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-3.pdf}
	\end{figure}
	\begin{enumerate}
		\item Il nodo \textsf{Bitmap Heap Scan} carica le righe indicate dall'indice;
		\item Per ciascuna riga caricata, ricontrolla che \textsf{id < 1000};
		\item Se inoltre \textsf{nomeins} inizia con \textsf{'Al'}, essa viene selezionata.
	\end{enumerate}\newpage

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di \textsf{explain} con uso di \textbf{due indici sulla stessa tabella}:
	\begin{lstlisting}[language=SQL]
...
EXPLAIN SELECT * FROM Insegn WHERE id < 1000 AND nomeins LIKE 'Al%'\end{lstlisting}
	...
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-4.pdf}
	\end{figure}
	\begin{itemize}
		\item \textbf{Ciascuna foglia scansiona un indice}. Il padre delle foglie (\textsf{BitmapAnd}) fa l'\textbf{intersezione degli indirizzi delle righe trovate}.
		\item La root (\textsf{Bitmap Heap Scan}) \textbf{carica le righe} ed \textbf{ricontrolla} che le \textbf{condizioni siano soddisfatte}.
	\end{itemize}\:\newline

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di \textsf{explain} con uso di \textbf{due indici sulla stessa tabella}:
	\begin{lstlisting}[language=SQL]
EXPLAIN SELECT * FROM Insegn WHERE id < 1000 OR nomeins LIKE 'Al%'\end{lstlisting}
	\emph{\underline{Output:}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-5.pdf}
	\end{figure}
	\begin{itemize}
		\item Il \textbf{padre delle foglie} (\textsf{BitmapOr}) esegue l'\textbf{unione degli indirizzi delle righe trovate}.
	\end{itemize}\newpage

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di \textsf{explain} con \textbf{planner che decide diversamente cambiando condizione}:
	\begin{lstlisting}[language=SQL]
EXPLAIN SELECT * FROM Insegn WHERE id < 1000 AND nomeins LIKE 'Al%'\end{lstlisting}
	\emph{\underline{Output:}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-6.pdf}
	\end{figure}
	\begin{itemize}
		\item La condizione \textsf{nomeins LIKE '\%Al'} rende \textbf{impossibile l'uso dell'indice}.
	\end{itemize}\:\newline

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di \textsf{explain} con uso di \textbf{un join mediante hash table}:
	\begin{lstlisting}[language=SQL]
EXPLAIN SELECT * FROM Insegn JOIN Inserogato IE ON IE.id_insegn = I.id
WHERE IE.annoaccademico = '2013/2014'\end{lstlisting}
	\emph{\underline{Output:}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-7.pdf}
	\end{figure}
	\begin{enumerate}
		\item Rimossi indici relativi ad \textsf{annoaccademico} prima di eseguire questa prova;
		\item Il nodo Hash prepara un hash table tramite una scansione sequenziale (\textsf{Seq Scan});
		\item Il nodo Hash Join, per ogni riga fornita dal primo figlio (\textsc{Seq Scan}), cerca nella hash table (secondo figlio) la riga da unire secondo la condizione (\textsf{IE.annoaccademico = '2013/2014'});
	\end{enumerate}
	\begin{itemize}
		\item Nonostante Insegn.id ha indice, qui non viene utilizzato!
		\item L'unico \textsf{\textbf{Seq Scan}} che si potrebbe \textbf{ottimizzare} è quello a riga 3.
	\end{itemize}\:\newpage

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di \textsf{explain} con uso di \textbf{un join mediante nested loop}:
	\begin{lstlisting}[language=SQL]
...
WHERE IE.annoaccademico = '2013/2014'\end{lstlisting}
	...
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-8.pdf}
	\end{figure}
	\begin{itemize}
		\item La condizione del \textbf{JOIN è meno selettiva} (le righe finali sono più di 14 milioni);
		\item Il join viene fatto usando un loop (\textsf{Nested Loop}): \textbf{ogni riga prodotta dal nodo \textsf{Seq Scan} si unisce con ogni riga prodotta dal nodo \textsf{INDEX Scan}};
		\item L'unico \textsf{\textbf{Seq Scan}} che si potrebbe \textbf{ottimizzare} è quello alla riga 2.
	\end{itemize}

	\noindent
	L'\textbf{ottimizzazione della query precedente}:
	\begin{lstlisting}[language=SQL]
...
WHERE IE.annoaccademico = '2013/2014'\end{lstlisting}
	...
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-10.pdf}
	\end{figure}
	\begin{itemize}
		\item Da 397.269 accessi a disco si è passati a 391.932!
	\end{itemize}\:\newpage

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di \textsf{explain} con uso di \textbf{un join mediante merge join}:
	\begin{lstlisting}[language=SQL]
EXPLAIN SELECT * FROM t1, t2
WHERE t1.unique1 < 100 AND t1.unique2 = t2.unique2\end{lstlisting}
	\emph{\underline{Output:}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-11.pdf}
	\end{figure}
	\begin{itemize}
		\item \textsf{Merge Join} esegue il join ordinando prima le due tabelle rispetto gli attributi di join;
		\item t1 è già ordinata via indice;
		\item t2 non è ordinata, quindi c'è un nodo Sort;
		\item L'unico \textbf{\textsf{Seq Scan}} che si potrebbe \textbf{ottimizzare} è quello evidenziato: un indice eviterebbe il \textsf{Sort}.
	\end{itemize}\newpage

	\subsubsection{Explain avanzato}

	Il comando \textcolor{Red3}{\textbf{\textsf{EXPLAIN ANALYZE}}}:
	\begin{enumerate}
		\item Mostra il piano di esecuzione;
		\item Esegue la query senza registrare eventuali modifiche;
		\item Mostra una stima verosimile dei tempi di esecuzione.
	\end{enumerate}
	Un \textcolor{Green4}{\textbf{esempio}} di \textsf{explain analyze} con esecuzione:
	\begin{lstlisting}[language=SQL]
EXPLAIN ANALYZE SELECT * FROM Insegn WHERE id < 1000 AND nomeins LIKE 'Algo %'\end{lstlisting}
	\emph{\underline{Output:}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-12.pdf}
	\end{figure}
	\begin{itemize}
		\item Per \textbf{ogni nodo del piano}: dettaglio righe rimosse, memoria usata e tempi di esecuzione in millisecondi (ms);
		\item Alla \textbf{fine}: stima tempo di pianificazione e di esecuzione.
	\end{itemize}\newpage

	\noindent
	Un altro \textcolor{Green4}{\textbf{esempio}} di \textsf{explain analyze} con esecuzione di una query pesante:
	\begin{lstlisting}[language=SQL]
EXPLAIN ANALYZE SELECT * FROM Insegn I, inserogato IE
WHERE IE.annoaccademico = '2013/2014' AND IE.id_insegn > I.id\end{lstlisting}
	\emph{\underline{Output:}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/explain-13.pdf}
	\end{figure}
	\begin{itemize}
		\item Il valore di \textsf{loops} indica per \textbf{quante volte viene eseguito il nodo};
		\item Spesso il numero di righe dopo cost è diverso dal numero di righe dopo \textsf{actual TIME}. Quest'\textbf{ultimo è più corretto}.
	\end{itemize}\newpage
	...
	Una \textcolor{Red3}{\textbf{transazione}} SQL è una \textbf{sequenza di istruzioni SQL} che può essere \textbf{eseguita in concorrenza con altre transazioni in modo isolato}. Come detto nei capitoli di teoria, un livello di isolamento troppo elevato potrebbe causare anomalie di esecuzione:
	\begin{itemize}
		\item Perdita di aggiornamento (\emph{lost update})
		\item Lettura sporca (\emph{dirty read})
		\item Letture inconsistenti (\emph{non-repeatable read})
		\item Aggiornamento fantasma (\emph{ghost update})
		\item Inserimento fantasma (\emph{phantom update})
	\end{itemize}
	Un'altra anomalia non considerata fin ad ora è la \textbf{mancata serializzazione} (\emph{\textbf{serialization anomaly}}). Ovvero, il \textbf{risultato di un gruppo di transazioni (quindi nessun \textsf{abort}) è inconsistente con tutti gli ordini di esecuzioni seriali delle stesse}.\newline

	\noindent
	In PostgreSQL una transazione:
	\begin{itemize}
		\item \textbf{Inizia} con il comando \textsf{BEGIN};
		\item \textbf{Termina} con il comando \textsf{COMMIT}, confermando tutte le istruzioni delle transazioni;
		\item \textbf{Annullata} con il comando \textsf{ROLLBACK}, quindi annullando tutte le operazioni.
	\end{itemize}\:\newline

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di transazione con passaggio di importo da un conto ad un altro:
	\begin{lstlisting}[language=SQL]
BEGIN;
UPDATE accounts SET balance = balance -100.0 WHERE name='Alice';
UPDATE accounts SET balance = balance -100.0 WHERE name='Alice';
COMMIT;\end{lstlisting}
	In questo caso, Alice e Bob vedono le modifiche sul proprio conto. Al contrario, in questo secondo \textcolor{Green4}{\textbf{esempio}}, le modifiche non vengono applicate:
	\begin{lstlisting}[language=SQL]
BEGIN;
UPDATE accounts SET balance = balance -100.0 WHERE name='Alice';
UPDATE accounts SET balance = balance -100.0 WHERE name='Alice';
ABORT;\end{lstlisting}\newpage

	\subsubsection{Controllo di concorrenza - Introduzione}\label{par: Controllo di concorrenza - Introduzione}

	PostgreSQL mantiene la consistenza dei dati usando un \textbf{modello multi-versione} (\textbf{Multiversion Concurrency Control (MVCC)}). Questo modello è una \textbf{metodologia più versatile della tecnica del locking a due fasi} (2PL stretto) tipica dei DBMS tradizionali.\newline

	\noindent
	In generale, le \textbf{transazioni sono gestite} nel seguente modo:
	\begin{itemize}
		\item \textbf{Ogni transazione vede un'istantanea della base di dati};
		\item \textbf{Letture} \textbf{\underline{sempre} possibili e mai bloccate} anche se ci sono altre transazioni che stanno modificando la base di dati;
		\item \textbf{Scritture} \textbf{sospese} quando in parallelo un'\textbf{altra transazione}, non ancora chiusa, ha \textbf{modificato la sorgente dei dati che si vuole aggiornare};
		\item \textbf{\textsf{COMMIT}} dei dati, il \textbf{sistema registra \underline{sempre} l'istantanea aggiornata come nuova base di dati}.
	\end{itemize}
	Con questa gestione, ci sono \textbf{meno conflitti} e una performance più ragionevole in ambito multiutente.\newpage

	\noindent
	Un \textcolor{Green4}{\textbf{esempio}} di sessioni concorrenti: si consideri la tabella \textsf{Studente} con il seguente contenuto:
	\begin{table}[!htp]
		\centering
		\begin{tabular}{@{} l | l | l @{}}
			\toprule
	...
	\noindent
	Si considerino le seguenti transazioni eseguite in parallelo. I numeri ai lati rappresentano gli istanti in cui si eseguono le istruzioni:
	\begin{table}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/concorrenza-1.pdf}
		\includegraphics[width=\textwidth]{img/concorrenza-2.pdf}
	\end{table}\newpage

	\subsubsection{Livelli di isolamento e cambiamento di isolamento}

	PostgreSQL offre \textbf{4 livelli di isolamento} in \underline{ordine decrescente di isolamento}:
	\begin{enumerate}
		\item \textcolor{Red3}{\textbf{Serializable}}, garantisce che un'intera \textbf{transazione} venga \textbf{eseguita in qualche ordine sequenziale rispetto ad altre transazioni concorrenti}.
		
		\underline{Anomalie possibili:} \textbf{\emph{nessuna}}.


		\item \textcolor{Red3}{\textbf{Repeatable Read}}, garantisce che i \textbf{dati letti durante la transazione non cambieranno a causa di altre transazioni}. Quindi, rieseguendo la lettura dei medesimi dati, si ottengono sempre gli stessi.
		
		\underline{Anomalie possibili:} \textbf{\emph{inserimento fantasma}}.

		
		\item \textcolor{Red3}{\textbf{Read Committed}}, garantisce che \textbf{qualsiasi \textsf{SELECT}} di una transazione \textbf{veda solo i dati confermati} (\textsf{COMMITTED}) \textbf{prima che la \textsf{SELECT} inizi}.
		
		\underline{Anomalie possibili:}
		\begin{itemize}
			\item \textbf{\emph{lettura inconsistente}}
			\item \textbf{\emph{aggiornamento fantasma}}
			\item \textbf{\emph{inserimento fantasma}}
			\item \textbf{\emph{mancata serializzazione}}
		\end{itemize}

		
		\item \textcolor{Red3}{\textbf{Read Uncommitted}} Viene implementato come Read Committed, quindi \textbf{non esiste}.
	\end{enumerate}
	Il \textbf{livello di default} utilizzato da PostgreSQL è \textbf{Read Committed} e il \textbf{cambio di livello di isolamento viene effettuato con \textsf{SET TRANSACTION} appena dopo il \textsf{BEGIN}} della transazione.\newline
	
	\noindent
	La \textbf{sintassi} del cambio di livello di isolamento con \textcolor{Red3}{\textbf{SET TRANSACTION}} è la seguente:
	\begin{itemize}
		\item Modalità standard:
		\begin{lstlisting}[language=SQL]
BEGIN;
...
COMMIT;\end{lstlisting}
		...
		\item Modalità compatta:
		\begin{lstlisting}[language=SQL]
BEGIN TRANSACTION ISOLATION LEVEL transaction_mode;
...
COMMIT;\end{lstlisting}
	\end{itemize}
	In cui \textsf{transaction\_mode} è uno dei seguenti valori:
	\begin{itemize}
		\item \textsf{SERIALIZABLE} pagina \pageref{par: Serializable}
		\item \textsf{REPEATABLE READ} pagina \pageref{par: Repeatable Read}
		\item \textsf{READ COMMITTED} (\emph{default}), pagina \pageref{par: Read Committed}
		\item \textsf{READ UNCOMMITTED}
		\item \textsf{READ WRITE}
		\item \textsf{READ ONLY}
	\end{itemize}
	
	\longline

	\subsubsection{Read Committed}\label{par: Read Committed}
	
	\textcolor{Red3}{\textbf{Read Committed}} è impostato di \emph{default} e quindi non necessita di essere specificato dopo il \textsf{BEGIN}. Con questo livello di isolamento:
	\begin{itemize}
		\item \textsf{SELECT} vede solo i dati registrati (COMMITTED) in altre transazioni e quelli modificati da eventuali comandi precedenti nella medesima transazione.
		
		\item \textsf{UPDATE}, \textsf{DELETE} lo stesso di \textsf{SELECT}. In aggiunta, se i dati da aggiornare sono stati modificati (\textsf{BEGIN}) ma non registrati (assenza di \textsf{COMMIT}) in transazioni correnti, allora:
		\begin{enumerate}
			\item Attesa che la transazione, che ha modificato i dati, vada a buon fine (comando \textsf{COMMIT}) o che venga annullata (comando \textsf{ROLLBACK});
			
			\item Riesaminate le righe selezionate per verificare che i criteri vengano soddisfatti ancora.
		\end{enumerate}
	\end{itemize}
	Come detto in precedenza, le \textbf{anomalie che si possono presentare} sono:
	\begin{itemize}
		\item \textbf{\emph{Lettura inconsistente}}
		\item \textbf{\emph{Aggiornamento fantasma}}
		\item \textbf{\emph{Inserimento fantasma}}
	...
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/read-committed.pdf}
		\caption{Caso lettura inconsistente con Read Committed.}
	\end{figure}
	
	\noindent
	La transazione 1 ha due letture diverse della stessa tabella senza averla modificata!\newpage
	
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/read-committed-1.pdf}
		\caption{Caso aggiornamento fantasma con Read Committed.}
	\end{figure}
	
	\noindent
	La base di dati soddisfa sempre il vincolo, ma per la transazione 1 no. Infatti, la transazione 1 ha perso un aggiornamento!\newline
	
	\noindent
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/read-committed-2.pdf}
		\caption{Caso di non eliminazione con Read Committed.}
	\end{figure}
	
	\noindent
	La \textsf{DELETE} eseguita al tempo $t_{4}$ non cancella la tupla \textsf{id=2} (selezionata utilizzando la condizione \textsf{WHERE hit=10}). Questo perché l'operazione di \textsf{UPDATE} eseguita al tempo $t_{3}$, blocca la \textsf{DELETE} fino al prossimo \textsf{COMMIT} (tempo $t_{6}$).
	
	Quindi, l'operazione di \textsf{DELETE} riesamina la tupla \textsf{id=2}, la clausola \textsf{WHERE hit=10} non vale più, di conseguenza la tupla non viene cancellata.\newpage

	\subsubsection{Repeatable Read}\label{par: Repeatable Read}
	
	Al contrario di Read Committed, in \textcolor{Red3}{\textbf{Repeatable Read}} i \textbf{comandi di una transazione vedono sempre gli stessi dati}. Quindi, PostgreSQL associa alla transazione un'istantanea della base di dati all'esecuzione del suo primo comando. Con questo livello di isolamento:
	\begin{itemize}
		\item Due \textsf{SELECT} identiche successive vedono sempre gli \textbf{stessi dati};
		
		\item Se \textsf{UPDATE} e \textsf{DELETE} devono \textbf{operare su dei dati che sono stati modificati ma non registrati in transazioni concorrenti}, allora è necessario attendere il:
		\begin{itemize}
			\item \textsf{ROLLBACK} della transazione in cui è stato fatto il cambio e \textbf{riesaminare le righe selezionate};
			
			\item \textsf{COMMIT} della transazione in cui è stato fatto il cambio e \textbf{bloccare le operazioni con l'errore} \textsf{ERROR}: \dquotes{could NOT serialize access due to concurrent \textsf{UPDATE}}.
		\end{itemize}
	\end{itemize}
	Come detto in precedenza, le \textbf{anomalie che si possono presentare} sono:
	\begin{itemize}
		\item \textbf{\emph{Mancata serializzazione}}
	\end{itemize}
	Inoltre, è necessario tenere in considerazione la \textbf{possibilità di transazioni abortite per aggiornamenti concorrenti}.\newline
	
	\noindent
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/repeatable-read-1.pdf}
		\caption{Caso di utilizzo di \textsf{COMMIT}/\textsf{ROLLBACK} con Repeatable Read.}
	\end{figure}\newpage
	
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/repeatable-read-2.pdf}
		\caption{Caso di mancata serializzazione con Repeatable Read.}
	\end{figure}
	
	\noindent
	La tabella contiene due valori identici: 11. Non 11 e 12 poiché il \textbf{risultato non è consistente con nessun ordine di esecuzione} delle due transazioni.\newpage

	\subsubsection{Serializable}\label{par: Serializable}
	
	Il livello di isolamento \textcolor{Red3}{\textbf{serializable}} è il più restrittivo, infatti \textbf{nessuna anomalia è concessa}. Utilizzando questo livello, si deve prevedere la possibilità di transazioni abortite per aggiornamenti concorrenti (come nel caso di Repeatable Read a pagina \pageref{par: Repeatable Read}).	
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/serializable-1.pdf}
		\caption{Caso di serializzazioni \underline{non} possibili.}
	\end{figure}
	
	\noindent
	Un \textbf{\underline{metodo} per decidere} se questo livello di isolamento è \textbf{necessario}, è il seguente:
	\begin{center}
		\emph{\textbf{Verificare se la selezione delle righe} di un} \textsf{UPDATE/INSERT/DELETE} \emph{\textbf{in una transazione può essere modificata in una transazione concorrente} (che può aggiungere/togliere o modificare le righe di interesse).}
	\end{center}
	
	\noindent
	Nonostante l'utilizzo di questo livello di isolamento rende semplice lo sviluppo di programmi SQL, dall'altra parte dichiarare tutte le transazioni \textsf{Serializable} determina una \textbf{limitazione alle prestazioni} (\emph{throughput}) di un DBMS.
	
	\longline

	\subsubsection{Controllo della concorrenza}
	
	È fondamentale prevedere e gestire gli errori di concorrenza che possono occorrere con livello di isolamento \dquotes{Repeatable Read} o \dquotes{Serializable}.\newline
	
	\noindent
	Nel caso in cui una \textbf{transazione fallisce}, il \underline{codice di fallimento} è \textsf{SQLSTATE = '40001'}. In cui \textsf{SQLSTATE} è una \textbf{variabile di ambiente} che si può leggere sia da programmi interni sia da programmi esterni.\newline
	
	\noindent
	L'\textbf{unica cosa da fare in questo caso è ritentare l'esecuzione della transazione}. Questo perché il \textbf{costo computazionale è minore} di quello che si avrebbe nel caso in cui si gestissero le transazioni usando i lock espliciti.\newpage
	
	\subsection{Query in Python}
	
	\subsubsection{Scrittura di una tabella in un file}
	
	Il file \textsf{scritturaDict.py}:
	\lstinputlisting[language=Python]{code/scritturaDict.py}
	Il file \textsf{tabellaSpesa.csv}:
	\lstinputlisting{code/tabellaSpesa.csv}
	Output su terminale:
	\lstinputlisting{code/output-1}\newpage
	
	\subsubsection{Avanzato - Scrittura di una tabella in un file}
	
	Il file \textsf{scritturaClasse.py} rappresenta un'evoluzione del file del precedente paragrafo. Si risolve l'errore sugli importi e si dimostra un uso più avanzato di JSON:
	\lstinputlisting[language=Python]{code/scritturaClasse.py}
	Il database compatto \textsf{database.json}:
	\lstinputlisting{code/database.json}\newpage
	
	\noindent
	Output su terminale:
	\lstinputlisting{code/output-2}\newpage
	
	\subsection{Accesso a una base di dati PostgreSQL con Python}
	
	\subsubsection{Introduzione a \textsf{psycopg2}}
	
	\textsf{psycopg2} è una libreria scritta quasi completamente in C e implementa la API per accedere da una base di dati esterna chiamata DB-API v2.0. Essa implementa i cursori \emph{client side} e \emph{server side}, comunicazioni asincrone, le notifiche e il comando \textsf{COPY}.
	
	\longline
	
	\subsubsection{DB-API v2.0: Connection}
	
	L'\textbf{accesso ad un database} avviene tramite un oggetto di tipo \textcolor{Red3}{\textbf{\textsf{Connection}}}. Il metodo \textsf{connect(...)} accetta i parametri necessari per la connessione e ritorna un oggetto \textsf{Connection}:
	\lstinputlisting[language=Python]{code/connection.py}
	La classe \textsf{Connection} ha alcuni metodi principali:
	\begin{itemize}
		\item \textcolor{Red3}{\textsf{cursor(...)}}, \textbf{ritorna un cursore} della base di dati. L'oggetto cursore verrà spiegato nel prossimo paragrafo a pagina \pageref{par: DB-API v2.0: Cursore}.
		
		\item \textcolor{Red3}{\textsf{commit(...)}}, \textbf{registra la transazione} corrente. \textbf{N.B.} nel caso di mancato commit, le eventuali modifiche o inserimenti verranno persi.
		
		\item \textcolor{Red3}{\textsf{rollback(...)}}, \textbf{abortisce la transazione} corrente.
		
		\item \textcolor{Red3}{\textsf{close(...)}}, \textbf{chiude la connessione} corrente. Implica un \textsf{rollback()} automatico delle operazioni non registrate.
		
		\item \textcolor{Red3}{\textsf{autocommit}}, proprietà \textsf{read/write}:
		\begin{itemize}
			\item \textsf{True}, ogni comando inviato è una \textbf{transazione isolata}.
			
			\item \textsf{False} (default), il primo comando inviato inizia una transazione che deve terminate con \textsf{commit()} o \textsf{rollback()}.
		\end{itemize}
		
		\item \textcolor{Red3}{\textsf{readonly}}, proprietà \textsf{read/write}:
		\begin{itemize}
			\item \textsf{True}, nella sessione si possono \textbf{inviare comandi di modifica dati}.
			
			\item \textsf{False} (default), \underline{non} si possono inviare comandi di modifica dati.
		\end{itemize}
		
		\item \textcolor{Red3}{\textsf{isolation\_level}}, proprietà \textsf{read/write}. \textbf{Modifica il livello di isolamento} per la prossima transazione. I valori ammessi sono:
		\begin{itemize}
			\item \textsf{READ UNCOMMITTED}
			\item \textsf{READ COMMITTED}
			\item \textsf{REPEATABLE READ}
			\item \textsf{SERIALIZABLE}
			\item \textsf{DEFAULT}
		\end{itemize}
		È \emph{good practice} assegnare questa variabile appena dopo la creazione della connessione:
		\begin{lstlisting}[language=Python]
connector = psycopg2.connect(...)
connector.isolation_level = 'REPEATABLE READ'\end{lstlisting}
	\end{itemize}\newpage
	
	\subsubsection{DB-API v2.0: Cursore}\label{par: DB-API v2.0: Cursore}
	
	Un \textcolor{Red3}{\textbf{cursore}} gestisce l'interazione con la base di dati. Mediante un cursore è possibile inviare un comando SQL, accedere all'esito e ai dati di risposta del comando.\newline
	
	\noindent
	La classe \textsf{Cursor} ha i seguenti metodi principali:
	\begin{itemize}
		\item \textcolor{Red3}{\textsf{execute(comando, parametri)}}, \textbf{prepara ed esegue} un \dquotes{\textsf{comando}} SQL usando i \dquotes{\textsf{parametri}}. I parametri devono essere una tupla o un dizionario (\textsf{dict}). Il comando non ritorna niente (\textsf{None}). I risultati della query si recuperano con \textsf{fetch*()}:
		\lstinputlisting[language=Python]{code/cursore-1.py}
		Dopo l'esecuzione di \textsf{execute}, il cursore è un iterabile sulla tabella risultato, quindi è possibile accedere alle tuple del risultato anche con un ciclo:
		\lstinputlisting[language=Python]{code/cursore-5.py}

		\item \textcolor{Red3}{\textsf{executemany(comando, parametri)}}, \textbf{prepara ed esegue} un \dquotes{\textsf{comando}} SQL per ciascun valore presente nella lista \dquotes{\textsf{parametri}}:
		\lstinputlisting[language=Python]{code/cursore-2.py}
		
		\item \textcolor{Red3}{\textsf{fetchone()}}, \textbf{ritorna \underline{una} tupla della tabella risultato}. Si può utilizzare dopo una \textsf{execute}. Se non esistono tuple, il valore di ritorno è \textsf{None}.
		\lstinputlisting[language=Python]{code/cursore-3.py}
		
		\item \textcolor{Red3}{\textsf{fetchmany(<numero>)}}, \textbf{ritorna una lista di tuple della tabella risultato} di lunghezza max \textsf{<numero>}. È possibile utilizzarla dopo una \textsf{execute("SELECT...")}. Se non ci sono tuple, ritorna una lista vuota.
		\lstinputlisting[language=Python]{code/cursore-4.py}
		
		\item \textcolor{Red3}{\textsf{rowcount}}, metodo in sola lettura. \textbf{Restituisce il numero di righe prodotte dall'ultimo comando}. Viene ritornato $-1$ nel caso in cui non è possibile determinare il valore.
		
		\item \textcolor{Red3}{\textsf{statusmessage}}, metodo in sola lettura. \textbf{Restituisce il messaggio ritornato dall'ultimo comando eseguito}:
		\lstinputlisting[language=Python]{code/cursore-6.py}
	\end{itemize}
	
	\longline
	
	\subsubsection{Utilizzo di psycopg2}
	
	La libreria psycopg2 può essere utilizzata in modo corretta se vengono seguiti i seguenti passaggi:
	\begin{enumerate}
		\item Apertura di una connessione tramite \textsf{conn = psycopg2.connect(...)}
		
		\item (Opzionale) Modifica delle proprietà di livello di isolamento, autocommit, readonly
		
		\item Creazione di un cursore tramite \textsf{cur = conn.cursor()}
		
		\item Esecuzione di una (\textsf{execute}) o più (\textsf{executemany}) query
		
		\item In caso di sessione non autocommit, esecuzione di \textsf{conn.commit()} in caso di salvataggio delle modifiche, altrimenti \textsf{conn.rollback()} per annullare
		
		\item Chiusura del cursore, \textsf{cur.close()}, e della connessione, \textsf{conn.close()}. Con il costrutto \textsf{with()} di Python è possibile eseguire un \textsf{commit} automatico e una \textsf{close} automatica del cursore:
		\lstinputlisting[language=Python]{code/cursore-7.py}
	\end{enumerate}\newpage
	
	\subsubsection{Esempio di modulo basato su psycopg2}
	
	Un esempio di modulo basato su psycopg2:
	\lstinputlisting[language=Python]{code/psycopg2-ex.py}
	Output dell'esecuzione:
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/output-3.pdf}
	\end{figure}
\end{document}