...
\begin{document}
	...
	\begin{equation*}
		...
		\end{array}
	\end{equation*}
	Quindi il \textbf{massimo valore assoluto} è $1$ e il \textbf{minimo valore assoluto} è $-1$.\newpage
	...
	\textcolor{Green4}{\textbf{\emph{Calcolare}}}
	\begin{equation*}
		\displaystyle\underset{\hspace{-.3em} D}{\int \hspace{-.5em} \int} e^{x+y} \: \mathrm{d}x \: \mathrm{d}y
	\end{equation*}
	\textcolor{Green4}{\textbf{\emph{dove $D$ è la regione interna al triangolo di vertici $\left(0,0\right)$, $\left(2,0\right)$ e $\left(1,1\right)$.}}}\newline

	\noindent
	Innanzitutto conviene rappresentare la regione che, come dice il testo, è un triangolo:
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=\textwidth]{img/grafico-ex6-esame1.pdf}
	\end{figure}

	\noindent
	Sul metodo di risoluzione si sceglie $y$-semplice, ovvero i due limiti $a,b$ sono rispettivamente $0$ e $1$, mentre le funzioni devono essere calcolate. La scelta è stata basata sul fatto che se fosse stato scelto $x$-semplice, quindi con limiti $x=0$ e $x=2$, il calcolo dell'integrale sarebbe stato complesso dato che da $x=0$ a $x=1$ si ha la funzione $b$, mentre da $x=1$ a $x=2$ si ha la funzione $a$ (e $a \ne b$!).\newline

	\noindent
	Per calcolare le equazioni delle due rette, si prende in considerazione due punti della retta e si applica la classica formula:
	\begin{equation*}
		\dfrac{x-x_{1}}{x_{2}-x_{1}} = \dfrac{y-y_{1}}{y_{2}-y_{1}}
	\end{equation*}
	\begin{equation*}
		\begin{array}{lcl}
			\text{Retta }b 	&\rightarrow& \dfrac{x-1}{0-1} = \dfrac{y-1}{0-1} \\ [.5em]
							&\rightarrow& -x+1 = -y+1 \\
							&\rightarrow& x = y \\ [1em]
			\text{Retta }a 	&\rightarrow& \dfrac{x-2}{1-2} = \dfrac{y-0}{1-0} \\ [.5em]
							&\rightarrow& -x+2 = y \\
							&\rightarrow& x = -y+2
		\end{array}
	\end{equation*}\newpage
	...
	\noindent
	Quindi la regione $D$ è possibile scriverla nel seguente modo:
	\begin{equation*}
		D = \left\{ \left(x,y\right) \in \mathbb{R}^{2} \: \left| \: \left(0 \le y \le 1\right) \land \left(y \le x \le -y+2\right)\right.\right\}
	\end{equation*}
	Adesso che la regione $D$ è stata definita, e sono evidenti gli estremi di integrazione, si procede con il calcolo dell'integrale:
	\begin{equation*}
		\begin{array}{lcl}
			&\rightarrow& \displaystyle\int_{0}^{1} \int_{y}^{-y+2} e^{x+y} \: \mathrm{d}x \: \mathrm{d}y \\ [1.5em]
			&\rightarrow& \displaystyle\int_{0}^{1} \int_{y}^{-y+2} e^{x} \cdot e^{y} \: \mathrm{d}x \: \mathrm{d}y \\ [1.5em]
			&\rightarrow& \displaystyle\int_{0}^{1} e^{y} \cdot \int_{y}^{-y+2} e^{x} \: \mathrm{d}x \: \mathrm{d}y \\ [1.5em]
			&\rightarrow& \displaystyle\int_{0}^{1} \left[e^{y} \cdot e^{x}\right]_{y}^{-y+2} \: \mathrm{d}y \\ [1.5em]
			&\rightarrow& \displaystyle\int_{0}^{1} \left[\left(e^{y} \cdot e^{-y+2}\right) - \left(e^{y} \cdot e^{y}\right)\right]_{y}^{-y+2} \: \mathrm{d}y \\ [1.5em]
			&\rightarrow& \displaystyle\int_{0}^{1} \left(e^{2} - e^{2y}\right) \: \mathrm{d}y \\ [1.5em]
			&\rightarrow& \left[e^{2}y - \dfrac{e^{2y}}{2}\right]_{0}^{1} \\ [1.5em]
			&\rightarrow& \left(e^{2}\cdot 1 - \dfrac{e^{2 \cdot 1}}{2}\right) - \left(e^{2} \cdot 0 - \dfrac{e^{2 \cdot 0}}{2}\right) \\ [1.5em]
			&\rightarrow& e^{2} - \dfrac{e^{2}}{2} + \dfrac{1}{2} \\ [1.5em]
			&\rightarrow& \dfrac{2e^{2} - e^{2}}{2} + \dfrac{1}{2} \\ [1.5em]
			&\rightarrow& \dfrac{e^{2}}{2} + \dfrac{1}{2}
		\end{array}
	\end{equation*}
	Quindi il risultato dell'integrale è: $\dfrac{e^{2}}{2} + \dfrac{1}{2}$.
\end{document}