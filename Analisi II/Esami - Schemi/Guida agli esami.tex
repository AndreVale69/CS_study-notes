...
\begin{document}
	...
	\subsubsection{Parametrizzazione retta tangente in un punto all'arco di ellisse}
	...
	\noindent
	Quindi, il \textbf{primo passo} dell'applicazione del completamento dei quadrati è il raggruppamento dei valori simili, ovverosia:
	\begin{equation*}
		\begin{array}{rcl}
			9x^{2} + 4y^{2} + 36x - 24y + 36 &=& 0 \\
		...
		\end{array}
	\end{equation*}
	Il \textbf{secondo passo} è prendere in considerazione i termini con le $x$, e poi quelli con le $y$, e cercare un quadrato. Ovvero sia un valore $c$ tale per cui il $\Delta$ (nella formula del calcolo di un'equazione di secondo grado $\Delta = b^{2} - 4 \cdot a \cdot c$) sia uguale a zero:
	\begin{equation*}
		\begin{array}{rcl}
	...
	\noindent
	Il \textbf{terzo passo} è riscrivere l'equazione con i nuovi valori $c$, ma per lasciare invariata l'equazione è necessario annullarli, ovvero scrivere $c-c$ (nessun problema, con manipolazioni algebriche si riuscirà ad evitare di ritornare al punto di inizio):
	\begin{equation*}
	...
	\noindent
	Il \textbf{quarto passo} è la parametrizzazione vera e propria\footnote{Link utili: \href{https://www.youmath.it/forum/analisi-2n/7786-coordinate-ellittiche.html}{coordinate ellittiche} e \href{https://www.youmath.it/forum/analisi-2n/70602-parametrizzazione-di-unellisse.html}{parametrizzazione di un'ellisse}}.
\end{document}