...
%				\left(x'\left(t\right)\right)^{2} = \left(\dfrac{1}{2} e^{\frac{t}{2}} \cos\left(t\right) - e^{\frac{t}{2}} \sin\left(t\right)\right) \left(\dfrac{1}{2} e^{\frac{t}{2}} \cos\left(t\right) - e^{\frac{t}{2}} \sin\left(t\right)\right) \\
%				\\
%				\left(y'\left(t\right)\right)^{2} = \left(\dfrac{1}{2} e^{\frac{t}{2}} \sin\left(t\right) + e^{\frac{t}{2}} \cos\left(t\right)\right) \left(\dfrac{1}{2} e^{\frac{t}{2}} \sin\left(t\right) + e^{\frac{t}{2}} \cos\left(t\right)\right) \\
%				\\
%				\left(z'\left(t\right)\right)^{2} = 0^{2} = 0
%			\end{cases} \\
%			\\
%			&\rightarrow& \begin{cases}
%				\left(x'\left(t\right)\right)^{2} = \dfrac{1}{4} e^{t} \cos^{2}\left(t\right) - 2 \cdot \dfrac{1}{2} e^{t} \cos\left(t\right) \sin\left(t\right) + e^{t}\sin^{2}\left(t\right) \\
%				\\
%				\left(y'\left(t\right)\right)^{2} = \dfrac{1}{4} e^{t} \sin^{2}\left(t\right) + 2 \cdot \dfrac{1}{2} e^{t} \sin\left(t\right) \cos\left(t\right) + e^{t}\cos^{2}\left(t\right)
%			\end{cases} \\
%			\\
%			&\rightarrow& \begin{cases}
%				\left(x'\left(t\right)\right)^{2} = \dfrac{1}{4} e^{t} \cos^{2}\left(t\right) - e^{t} \cos\left(t\right) \sin\left(t\right) + e^{t}\sin^{2}\left(t\right) \\
%				\phantom{\left(x'\left(t\right)\right)^{2} = \dfrac{1}{4} e^{t} \cos^{2}\left(t\right) - e^{t} } \updownarrow \text{ si annullano!}\\
%				\left(y'\left(t\right)\right)^{2} = \dfrac{1}{4} e^{t} \sin^{2}\left(t\right) + e^{t} \sin\left(t\right) \cos\left(t\right) + e^{t}\cos^{2}\left(t\right)
%			\end{cases} \\
%			\\
%			\left(x'\left(t\right)\right)^{2} + \left(y'\left(t\right)\right)^{2} &=& %
%			\dfrac{5}{4} e^{t} \cos^{2}\left(t\right) + \dfrac{5}{4} e^{t} \sin^{2}\left(t\right) \\
%			\\
%			&=& \dfrac{5}{4} e^{t}\left(\cos^{2}\left(t\right) + \sin^{2}\left(t\right)\right) \\
%			\\
%			&=& \dfrac{5}{4}e^{t}
%		\end{array}
%	\end{equation*}
	...
	\subsubsection{Trovare il massimo/minimo assoluto della funzione nel caso sia soggetta ad un vincolo}
	...
	\begin{equation*}
		...
		\end{cases}
	\end{equation*}
	Si analizzano le equazioni a disposizione per cercare le soluzioni. Per evitare di perdersi o di ragionare su soluzioni già prese in considerazione, si consiglia di fare una tabella in cui le colonne identificano le incognite e le righe il numero di possibili valori che possono prendere nello spazio delle soluzioni. Il numero di righe è dato dalla somma dei gradi massimi di ogni riga del sistema, per il grado di ogni riga ($2\cdot 1+2 \cdot 1+2 \cdot 2$ in questo caso).\newpage
	...
	\begin{table}[!htp]
		...
		\begin{tabular}{@{} l | c | c | c | c @{}}
			\toprule
			& $x$ & $y$ & $\lambda$ & N° Equazione \\
			\midrule
			A & $0$ & $\dfrac{3}{\sqrt{2}}$ & $\dfrac{9}{2}$ & \multirow{2}{*}{1°} \\ [1em]
			B & $0$ & $-\dfrac{3}{\sqrt{2}}$ & $\dfrac{9}{2}$ & \\ [1em]
			\cmidrule{1-5}
			C & ? & ? & ? & \multirow{2}{*}{2°}\\ [1em]
			D & ? & ? & ? &  \\ [1em]
			\cmidrule{1-5}
	...
	\begin{table}[!htp]
		...
		\begin{tabular}{@{} l | c | c | c | c @{}}
			...
			B & $0$ & $-\dfrac{3}{\sqrt{2}}$ & $\dfrac{9}{2}$ & \\ [1em]
			\cmidrule{1-5}
			C & $3$ & $0$ & $4$ & \multirow{2}{*}{2°}\\ [1em]
			D & $-3$ & $0$ & $4$ &  \\ [1em]
			\cmidrule{1-5}
			E & $\sqrt{5}$ 	& $\sqrt{2}$ 	& $2$ & \multirow{4}{*}{3°} \\  [1em]
	...
	\begin{table}[!htp]
		...
		\begin{tabular}{@{} l | c | c | c | c @{}}
			...
			B & ? & ? & ? & \\
			\cmidrule{1-5}
			C & $2\sqrt{2}$ & $0$ & $2+\sqrt{2}$ & \multirow{4}{*}{2°}\\ 
			D & $2\sqrt{2}$ & $0$ & $2-\sqrt{2}$ &  \\ 
			E & $-2\sqrt{2}$ & $0$ & $2+\sqrt{2}$ &  \\  
			F & $-2\sqrt{2}$ & $0$ & $2-\sqrt{2}$ & \\ 
			\cmidrule{1-5}
			G & ? & ? & ? & \multirow{2}{*}{3°} \\ 
			H & ? & ? & ? & \\
			\bottomrule
	...
	\begin{table}[!htp]
		...
		\begin{tabular}{@{} l | c | c | c | c @{}}
			...
			F & $-2\sqrt{2}$ & $0$ & $2-\sqrt{2}$ & \\ 
			\cmidrule{1-5}
			G & $\dfrac{\sqrt{28}}{3}$ & $-\dfrac{4}{3}$ & $\dfrac{4}{3}$ & \multirow{2}{*}{3°} \\ [1em]
			H & $-\dfrac{\sqrt{28}}{3}$ & $-\dfrac{4}{3}$ & $\dfrac{4}{3}$ & \\
			\bottomrule
		\end{tabular}
		\caption*{Tabella delle soluzioni.}
	\end{table}
