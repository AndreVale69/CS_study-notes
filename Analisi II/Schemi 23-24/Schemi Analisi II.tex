...
\begin{document}
	...
	% 	y'' + y' = cos(x)+2
	% \end{equation*}
	% Viene utilizzato il metodo di somiglianza per risolvere questa equazione differenziale, ma viene utilizzato anche il principio di % sovrapposizione. Prima di tutto vengono calcolate le soluzioni dell'equazione caratteristica:
	% \begin{equation*}
	% 	\dfrac{-1 \pm 1}{2} \longrightarrow \lambda_{1} = 0 \hspace{.5em} \lambda_{2} = -1
	% \end{equation*}
	% L'integrale generale corrisponde a:
	% \begin{equation*}
	% 	y\left(x\right) = y_{P}\left(x\right) + c_{1} e^{0 \cdot x} + c_{2}e^{-x} = y_{P}\left(x\right) + c_{2}e^{-x}
	% \end{equation*}
