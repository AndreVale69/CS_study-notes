...
\begin{document}
	...
	\begin{flushleft}
		\label{exam: esame 03 marzo 2023 - Gruppo A - 6 esercizio}
		\hypertarget{
			exam: esame 03 marzo 2023 - Gruppo A - 6 esercizio
		}{
			\definition{\underline{Esame 03 marzo 2023 - Gruppo A - 6 esercizio}}
		}
	\end{flushleft}
	\example{\emph{Sia $D$ la regione del piano limitata dalla curva $y = -\sqrt[3]{4x}$, dal segmento che unisce i punti $\left(-1, 0\right)$ e $\left(-2,2\right)$ e dall'asse $x$.}}
	\begin{figure}[!htp]
		\centering
		\includegraphics[width=.6\textwidth]{img/exercise/2023-03-03-A-ex6.png}
	\end{figure}

	\noindent
	\example{\emph{Calcolare $\displaystyle\iint\limits_{D} y \:\mathrm{d}x\:\mathrm{d}y$}}

	\noindent
	\example{\emph{Sapendo che l'area della regione $D$ è $2$, determinare la quota $y$ del baricentro di $D$.}}\newline

	\noindent
	Qualsiasi tecnica si utilizzi per calcolare l'integrale, è necessario ottenere l'equazione della retta passante per i punti $\left(-1,0\right)$ e $\left(-2,2\right)$. Si utilizza dunque la classica formula:
	\begin{equation*}
		\dfrac{x-x_{1}}{x_{2}-x_{1}} = \dfrac{y-y_{1}}{y_{2}-y_{1}} \rightarrow \dfrac{x-\left(-1\right)}{-2-\left(-1\right)} = \dfrac{y-0}{2-0} \rightarrow -2x-2 = y
	\end{equation*}
	Per capire quale sia il verso della disuguaglianza, si esegue il classico test con un punto di prova. Per esempio, dato il punto $\left(-1,1\right)$, si desidera che la disuguaglianza sia vera, per cui:
	\begin{equation*}
		-2\left(-1\right)-2 = 1 \rightarrow 0 = 1 \Longrightarrow 0 \le 1 \Longrightarrow -2x-2 \le y
	\end{equation*}
	Per quanto riguarda il metodo di risoluzione, sembrerebbe conveniente usare $x$-semplice poiché si bloccano i valori di $y$ che sono banalmente $0 \le y \le 2$. Per quanto riguarda le due funzioni sulle quali integrare $x$, nessun tipo di problema, quella di partenza (la retta) è già stata calcolata, mentre la curva viene fornita dall'esercizio. Dunque, per creare l'insieme $D$ si esplicita la $x$ nella disuguaglianza della retta e il medesimo ragionamento si effettua anche per la curva (ovviamente prima si ragiona su come convertirla in disuguaglianza).\newline

	\noindent
	Si esplicita la $x$ nella disuguaglianza della retta:
	\begin{equation*}
		-2x-2 \le y \Longrightarrow x \ge -\dfrac{y + 2}{2}
	\end{equation*}
	Per quanto riguarda la curva, si inizia esplicitando la $x$:
	\begin{equation*}
		y = -\sqrt[3]{4x} \Longrightarrow y^{3} = -4x \Longrightarrow -\dfrac{y^{3}}{4} = x
	\end{equation*}
	E per trovare la disuguaglianza si ragiona come si è fatto per la retta. Il punto di prova $\left(-1, 1\right)$ deve essere all'interno, per cui:
	\begin{equation*}
		-\dfrac{1^{3}}{4} = -1 \Longrightarrow -\dfrac{1}{4} \ge -1 \Longrightarrow -\dfrac{y^{3}}{4} \ge x
	\end{equation*}
	Adesso si hanno tutte le risorse per creare l'insieme $D$:
	\begin{equation*}
		D = \left\{\left(x,y\right) \in \mathbb{R}^{2} \: : \: 0 \le y \le 2, \: -\dfrac{y+2}{2} \le x \le -\dfrac{y^{3}}{4}\right\}
	\end{equation*}
	Per vedere la figura e le sue coordinate in maniera più dettagliate, aprire il seguente \href{https://www.geogebra.org/calculator/xnqawhvt}{link di GeoGebra} o scannerizzare il seguente QR code:
	\begin{center}
		\qrcode{https://www.geogebra.org/calculator/xnqawhvt}
	\end{center}
	Si calcola l'integrale:
	\begin{equation*}
		\begin{array}{rcl}
			\displaystyle\iint\limits_{D} y \:\mathrm{d}x\:\mathrm{d}y
			&=&
			\displaystyle\int_{0}^{2} \left(\int_{-\frac{y+2}{2}}^{-\frac{y^{3}}{4}}y \:\mathrm{d}x\right) \:\mathrm{d}y \\ [2em]
			%
			&=&
			\displaystyle\int_{0}^{2} y \left(-\dfrac{y^{3}}{4} + \dfrac{y+2}{2}\right) \:\mathrm{d}y \\ [1.5em]
			%
			&=&
			\displaystyle\int_{0}^{2} - \dfrac{1}{4}y^{4} + \dfrac{1}{2}y^{2} + y \:\mathrm{d}y \\ [1.5em]
			%
			&=&
			-\dfrac{1}{4}\left(\dfrac{2^{5}}{5}\right) + \dfrac{1}{2}\left(\dfrac{2^{3}}{3}\right) + \left(\dfrac{2^{2}}{2}\right) \\ [1.5em]
			%
			&=&
			-\dfrac{32}{20} + \dfrac{8}{6} + \dfrac{4}{2} \\ [1em]
			%
			&=&
			\dfrac{26}{15}
		\end{array}
	\end{equation*}
	La coordinata di $y$ del baricentro si calcola con la classica formula:
	\begin{equation*}
		y_{G} 
		= 
		\dfrac{\displaystyle\iint\limits_{D} y \:\mathrm{d}x\:\mathrm{d}y}{\displaystyle\iint\limits_{D} \:\mathrm{d}x\:\mathrm{d}y}
		=
		\dfrac{\frac{26}{15}}{2}
		=
		\dfrac{13}{15}
	...
	\begin{flushleft}
		\label{exam: esame 03 marzo 2023 - Gruppo A - 7 esercizio}
		\hypertarget{
			exam: esame 03 marzo 2023 - Gruppo A - 7 esercizio
		}{
			\definition{\underline{Esame 03 marzo 2023 - Gruppo A - 7 esercizio}}
		}
	\end{flushleft}
	\example{\emph{Si consideri la regione}
	\begin{equation*}
		\Omega = \left\{\left(x,y,z\right) \in \mathbb{R}^{3} \: : \: 0 \le x \le 1, \: 0 \le y \le x, \: 0 \le z \le x-y\right\}
	\end{equation*}
	\emph{Calcolare}
	\begin{equation*}
		\displaystyle\iiint\limits_{\Omega} \left(x-\cos\left(z\right)\right) \:\mathrm{d}x\:\mathrm{d}y\:\mathrm{d}z
	\end{equation*}
	\emph{e dare una possibile interpretazione fisica al risultato trovato.}}

	\noindent
	La regione è già definita, per cui si procede per fili considerando prima la $z$ e successivamente si integra per $y$-semplice:
	\begin{equation*}
		\begin{array}{rcl}
			\displaystyle\iiint\limits_{\Omega} \left(x-\cos\left(z\right)\right) \:\mathrm{d}x\:\mathrm{d}y\:\mathrm{d}z
			&=&
			\displaystyle\int_{0}^{1} \left(\int_{0}^{x} \left(\int_{0}^{x-y} x-\cos\left(z\right) \:\mathrm{d}z\right) \:\mathrm{d}y\right) \:\mathrm{d}x \\ [1.5em]
			%
			&=&
			\displaystyle\int_{0}^{1} \left(\int_{0}^{x} \left(x^{2}-xy -\sin\left(x-y\right)\right) \:\mathrm{d}y\right) \:\mathrm{d}x \\ [1.5em]
			%
			&=&
			\displaystyle\int_{0}^{1} \left(x^{3} - \dfrac{1}{2}x^{3} - 1 + \cos\left(x\right)\right) \:\mathrm{d}x \\ [1.5em]
			%
			&=&
			\dfrac{1^{4}}{4} - \dfrac{1}{2}\left(\dfrac{1^{4}}{4}\right) - 1 + \sin\left(1\right) \\ [1em]
			%
			&=&
			-\dfrac{7}{8} + \sin\left(1\right)
		\end{array}
	\end{equation*}
	Una possibile interpretazione è: l'integrale rappresenta la densità di massa di un corpo nello spazio $\Omega$.
	...
	\begin{boxdef}
		...
		\definition{Attenzione:} \textbf{se il cammino $\gamma$ è chiuso, l'integrale ha valore $0$.}
	\end{boxdef}
	...
	\begin{itemize}
		\item Il più semplice è utilizzare la differenza di potenziali. Il potenziale si è ottenuto a pagina~\hyperlink{
			...
			\hypergetpageref{exam: esame 21 giugno 2023 - Gruppo A - 8 esercizio (a)}
	...
	\subsubsection{Calcolo dell'integrale di linea e del teorema di Green}

	\begin{flushleft}
		\label{exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (a)}
		\hypertarget{
			exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (a)
		}{
			\definition{\underline{Esame 03 marzo 2023 - Gruppo A - 8 esercizio (\emph{a})}}
		}
	\end{flushleft}
	\example{\emph{Si consideri il campo veottoriale $\overset{\rightarrow}{F}\left(x,y\right) = \left(x+y, y-x\right)$ e il cammino \textbf{chiuso} $\gamma = \gamma_{1} \cup \gamma_{2}$, dove $\gamma_{1}\left(t\right) = \left(t, t^{2}-1\right)$, con $t \in \left[1,2\right]$ (arco di parabola) e $\gamma_{2}$ è il segmento che unisce i punti $\left(2,3\right)$ e $\left(1,0\right)$.}}

	\noindent
	\example{\emph{Calcolare $\oint_{\gamma} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r}$. Il risultato ottenuto ci permette di affermare che il campo non è conservativo? (motivare la risposta).}}\newline

	\noindent
	Il primo passo è calcolare la parametrizzazione $\gamma_{2}\left(t\right)$ che è una retta:
	\begin{equation*}
		\gamma_{2}\left(t\right) = \left(2,3\right) + t\left(\left(1,0\right) - \left(2,3\right)\right) = \left(2-t, 3-3t\right) \hspace{2em} t \in \left[0,1\right]
	\end{equation*}
	Per calcolare l'integrale di linea di prima specie lungo $\gamma$, si può fare facilmente, ma ricordando che $\gamma$ è l'unione, per cui:
	\begin{equation*}
		\displaystyle\oint_{\gamma} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r} = \displaystyle\oint_{\gamma_{1}} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r} + \displaystyle\oint_{\gamma_{2}} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r}
	\end{equation*}
	Si calcola prima l'integrale di linea lungo $\gamma_{1}\left(t\right)$:
	\begin{equation*}
		\begin{array}{rcl}
			\displaystyle\oint_{\gamma_{1}} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r}
			&=&
			\displaystyle\int_{1}^{2} \overset{\rightarrow}{F}\left(\gamma_{1}\left(t\right)\right) \cdot \gamma_{1}'\left(t\right) \mathrm{d}t \\ [1.5em]
			%
			&=&
			\displaystyle\int_{1}^{2} \left(t^{2}+t-1, t^{2}-t-1\right)\left(1,2t\right) \mathrm{d}t \\ [1.5em]
			%
			&=&
			\displaystyle\int_{1}^{2} 2t^{3}-t^{2}-t-1 \mathrm{d}t \\ [1.5em]
			%
			&=&
			2\left(\dfrac{2^{4}}{4} - \dfrac{1^{4}}{4}\right) - \left(\dfrac{2^{3}}{3} - \dfrac{1^{3}}{3}\right) - \left(\dfrac{2^{2}}{2} - \dfrac{1^{2}}{2}\right) - 1 \\ [1.5em]
			%
			&=&
			\dfrac{8}{3}
		\end{array}
	\end{equation*}
	Si calcola il secondo integrale di linea lungo $\gamma_{2}\left(t\right)$:
	\begin{equation*}
		\begin{array}{rcl}
			\displaystyle\oint_{\gamma_{2}} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r}
			&=&
			\displaystyle\int_{0}^{1} \overset{\rightarrow}{F}\left(\gamma_{2}\left(t\right)\right) \cdot \gamma_{2}'\left(t\right) \mathrm{d}t \\ [1.5em]
			%
			&=&
			\displaystyle\int_{0}^{1} \left(5-4t, 1-2t\right)\left(-1,-3\right) \mathrm{d}t \\ [1em]
			%
			&=&
			\displaystyle\int_{0}^{1} -8+10t \mathrm{d}t \\ [1em]
			%
			&=&
			-8\left(1\right) + 10\left(\dfrac{1^{2}}{2}\right) \\ [1em]
			%
			&=&
			-3
		\end{array}
	\end{equation*}
	Per cui l'integrale di linea è:
	\begin{equation*}
		\displaystyle\oint_{\gamma} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r} = \dfrac{8}{3} + \left(- 3\right) = -\dfrac{1}{3}
	\end{equation*}
	Per definizione, un campo non è conservativo se l'integrale di linea in un percorso chiuso è diverso da zero.

	\longline

	\begin{flushleft}
		\label{exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (b)}
		\hypertarget{
			exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (b)
		}{
			\definition{\underline{Esame 03 marzo 2023 - Gruppo A - 8 esercizio (\emph{b})}}
		}
	\end{flushleft}
	\example{\emph{Verificare che, in accordo con il teorema di Green, si ha}
	\begin{equation*}
		\displaystyle\oint_{\gamma} \overset{\rightarrow}{F} \cdot \mathrm{d}\overset{\rightarrow}{r} = \displaystyle\iint\limits_{D} \left(\dfrac{\partial F_{2}}{\partial x} - \dfrac{\partial F_{1}}{\partial y}\right) \:\mathrm{d}x\:\mathrm{d}y
	\end{equation*}
	\emph{dove $F_{1}$ e $F_{2}$ sono le componenti scalari del campo $\overset{\rightarrow}{F}$ e $D$ è la regione limitata da $\gamma$.}}\newline

	\noindent
	Questo esercizio è la seconda parte di un esercizio più grande, per cui si consiglia di vedere e far riferimento ai dati della prima parte a pagina~\hyperlink{
		exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (a)
	}{
		\hypergetpageref{exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (a)}%
	}.\newline

	\noindent
	Si calcolano prima le derivate parziali rispetto alle componenti del campo:
	\begin{gather*}
		\begin{array}{rcl}
			\dfrac{\partial F_{2}}{\partial x} &=& \dfrac{\partial }{\partial x}\left(y-x\right) = -1 \\ [1em]
			\dfrac{\partial F_{1}}{\partial y} &=& \dfrac{\partial }{\partial y}\left(x+y\right) = 1
		\end{array}
		\\
		\displaystyle\iint\limits_{D} \left(\dfrac{\partial F_{2}}{\partial x} - \dfrac{\partial F_{1}}{\partial y}\right) \:\mathrm{d}x\:\mathrm{d}y
		=
		\displaystyle\iint\limits_{D} -1 -1 \:\mathrm{d}x\:\mathrm{d}y 
		= 
		\displaystyle\iint\limits_{D} -2 \:\mathrm{d}x\:\mathrm{d}y
	\end{gather*}
	L'insieme $D$ si ricava dalla parametrizzazione (ottenendo le loro equazioni cartesiane):
	\begin{gather*}
		D = \left\{\left(x,y\right) \in \mathbb{R}^{2} \: : \: 1 \le x \le 2, \: x^{2}-1 \le y \le 3x-3\right\}
		\\
		\displaystyle\iint\limits_{D} -2 \:\mathrm{d}x\:\mathrm{d}y
		=
		-2\displaystyle\int_{1}^{2} \left(\int_{x^{2}-1}^{3x-3} \:\mathrm{d}y\right) \:\mathrm{d}x
		=
		-\dfrac{1}{3}
	\end{gather*}
			...
			\begin{itemize}
				...
				\begin{enumerate}[label*=Ex. \arabic*), leftmargin = 4em]
					...
					\item (\emph{a}) Determinare massimo e minimo assoluto con metodo di Lagrange: pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 03 marzo 2023 - Gruppo A - 5 esercizio (a)}
					}

					\setcounter{enumi}{4}

					\item (\emph{b}) Dimostrare che un punto è un punto di sella:\newline%
					pag.~\hyperlink{
						exam: esame 03 marzo 2023 - Gruppo A - 5 esercizio (b)
					}{
						\hypergetpageref{exam: esame 03 marzo 2023 - Gruppo A - 5 esercizio (b)}
					}

					\item Calcolare l'integrale doppio data una figura e calcolare il baricentro: pag.~\hyperlink{
						exam: esame 03 marzo 2023 - Gruppo A - 6 esercizio
					}{
						\hypergetpageref{exam: esame 03 marzo 2023 - Gruppo A - 6 esercizio}
					}

					\item Calcolare l'integrale triplo: pag.~\hyperlink{
						exam: esame 03 marzo 2023 - Gruppo A - 7 esercizio
					}{
						\hypergetpageref{exam: esame 03 marzo 2023 - Gruppo A - 7 esercizio}
					}

					\item (\emph{a}) Determinare l'integrale di linea unendo due parametrizzazioni: pag.~\hyperlink{
						exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (a)
					}{
						\hypergetpageref{exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (a)}
					}
					...
					\item (\emph{b}) Applicare il teorema di Green: pag.~\hyperlink{
						exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (b)
					}{
						\hypergetpageref{exam: esame 03 marzo 2023 - Gruppo A - 8 esercizio (b)}
					}
				\end{enumerate}
			\end{itemize}
			...
			\begin{itemize}
				...
				\begin{enumerate}[label*=Ex. \arabic*), leftmargin = 4em]
					...
					\item (\emph{a}) Determinare e classificare delle costanti per trovare un punto stazionario: pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 21 giugno 2023 - Gruppo A - 5 esercizio (a)}
					}
					...
					\item (\emph{b}) Determinare massimo e minimo assoluto: pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 21 giugno 2023 - Gruppo A - 5 esercizio (b)}
					}

					\item Calcolare l'integrale doppio con coordinate polari:\newline%
					pag.~\hyperlink{
						exam: esame 21 giugno 2023 - Gruppo A - 6 esercizio
					}{
						\hypergetpageref{exam: esame 21 giugno 2023 - Gruppo A - 6 esercizio}
					}

					\item Calcolare l'integrale triplo data un'immagine e con insieme da calcolare: pag.~\hyperlink{
						exam: esame 21 giugno 2023 - Gruppo A - 7 esercizio
					}{
						\hypergetpageref{exam: esame 21 giugno 2023 - Gruppo A - 7 esercizio}
					}

					\item (\emph{a}) Determinare se il campo è conservativo e il suo potenziale: pag.~\hyperlink{
						exam: esame 21 giugno 2023 - Gruppo A - 8 esercizio (a)
					}{
						\hypergetpageref{exam: esame 21 giugno 2023 - Gruppo A - 8 esercizio (a)}
					}
					...
					\item (\emph{b}) Calcolo dell'integrale in due modi diversi: lungo la parametrizzazione e differenza tra il potenziale valutato agli estremi: pag.~\hyperlink{
						exam: esame 21 giugno 2023 - Gruppo A - 8 esercizio (b)
					}{
						\hypergetpageref{exam: esame 21 giugno 2023 - Gruppo A - 8 esercizio (b)}
					}
				\end{enumerate}
			\end{itemize}
			...
			\begin{itemize}
				...
				\begin{enumerate}[label*=Ex. \arabic*), leftmargin = 4em]
					...
					pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 10 luglio 2023 - Gruppo A - 5 esercizio (a)}
					}
					...
					\item (\emph{b}) Determinare massimo e minimo assoluto: pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 10 luglio 2023 - Gruppo A - 5 esercizio (b)}
					}

					\item Calcolare l'integrale doppio definito in un triangolo con un insieme definito: pag.~\hyperlink{
						exam: esame 10 luglio 2023 - Gruppo A - 6 esercizio
					}{
						\hypergetpageref{exam: esame 10 luglio 2023 - Gruppo A - 6 esercizio}
					}

					\item Calcolare l'integrale triplo: pag.~\hyperlink{
						exam: esame 10 luglio 2023 - Gruppo A - 7 esercizio
					}{
						\hypergetpageref{exam: esame 10 luglio 2023 - Gruppo A - 7 esercizio}
					}
					...
					pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 06 settembre 2023 - Gruppo A - 8 esercizio (a)}
					}
					...
					\item (\emph{b}) Calcolo dell'integrale del campo lungo la parametrizzazione: pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 06 settembre 2023 - Gruppo A - 8 esercizio (b)}
					}
				\end{enumerate}
			\end{itemize}
			...
			\begin{itemize}
				...
				\begin{enumerate}[label*=Ex. \arabic*), leftmargin = 4em]
					...
					pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 06 settembre 2023 - Gruppo A - 5 esercizio (a)}
					}
					...
					\item (\emph{b}) Determinare massimo e minimo assoluto - Insieme chiuso ma non limitato: pag.~\hyperlink{
						...
						\hypergetpageref{exam: esame 06 settembre 2023 - Gruppo A - 5 esercizio (b)}
					}

					\item Calcolare il baricentro della regione piana rappresentata in figura: pag.~\hyperlink{
						exam: esame 06 settembre 2023 - Gruppo A - 6 esercizio
					}{
						\hypergetpageref{exam: esame 06 settembre 2023 - Gruppo A - 6 esercizio}
					}

					\item Calcolare l'integrale triplo: pag.~\hyperlink{
						exam: esame 06 settembre 2023 - Gruppo A - 7 esercizio
					}{
						\hypergetpageref{exam: esame 06 settembre 2023 - Gruppo A - 7 esercizio}
					}
